...
Bar::Bar(WINDOW *win, int x, int y, int width, int height, std::string title) : Displayable(win, x, y, width, height, title)
{
}
...
void Bar::Draw(float percent)
{
    ...
    if (m_customDispContent.length() == 0)
    {
        ...
        mvwprintw(m_win, y, x + m_width - 1 - str.length(), "%s%%", str.c_str());
    }
    else
    {
        mvwprintw(m_win, y, x + m_width - m_customDispContent.length(), "%s", m_customDispContent.c_str());
    }
}
